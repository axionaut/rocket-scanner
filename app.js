...
const SELL_PRESSURE_EDGE_MIN=0.08; // DMI- must beat DMI+ by this normalized edge to count as supply pressure.
const RVOL_PRESSURE_MIN=1.5; // Above this, raw RVOL must have upward direction or it is treated as distribution.
...
function updateModeUI(){
  ...
  if(brand) brand.textContent='Rocket mRMR + Rocket Lab';
...
let ENGINE_DATA={};
let REMOVED={uc:0,surv:0,liq:0,fscore:0,atr:0,deliv:0,fade:0,supply:0};
let SUPPRESSED_HELD=0; // count of stocks hidden because already held in POSITIONS
let SUPPRESSED_YDAY_ROCKET=0; // count of stocks hidden because they were rockets yesterday
...
let PREV_SNAP_META={}; // metadata for PREV_SNAP, including source date and market breadth
...
const CORR_SCHEMA='fresh_lagged_primary_v1';
...
const ROCKET_LAB_STORE='rs_rocket_lab_v1';
...
const FS = (() => {
  ...
  function canonicalInputName(name){
    ...
    if(n.includes('all nse')&&n.endsWith('.csv')) return 'ALL NSE.csv';
    if(n.endsWith('.zip')) return 'Reports-Daily-Multiple.zip';
    if(n.includes('holding')&&n.endsWith('.csv')) return 'Holdings.csv';
    if(n.includes('position')&&n.endsWith('.csv')) return 'Positions.csv';
    if(n.includes('order')&&n.endsWith('.csv')) return 'Orders.csv';
    if(n.includes('tradebook')&&n.endsWith('.csv')) return 'TRADEBOOK.csv';
    if((n.includes('nse holidays')||n.includes('nse_holidays'))&&n.endsWith('.csv')) return 'NSE Holidays.csv';
    return null;
  }
...
// Rocket % is only the rocket-label threshold. Runway uses STOCK_RUNWAY_CEILING_PCT.
// Retained as no-op for call sites in engine post-run, loadFilterState, clearFilters.
function onRocketPctChange(resolvedThreshold){}

// User-facing: rocket threshold changed via filter bar input.
// Updates rocket labels/counts immediately. Learning impact only takes effect on next CSV upload.
function onRocketPctChangeUI(){
  const el=document.getElementById('fRocketPct');
  const raw=parseFloat(el?.value);
  const v=(isFinite(raw)&&raw>=5&&raw<=20)?raw:10;
  if(ENGINE_DATA) ENGINE_DATA.rocketThreshold=v;
...
function sanitizeSavedMethodology(meth){
  if(!meth||!meth.removed) return meth;
  const rows=Array.isArray(meth.survRuleRows)?meth.survRuleRows:[];
  const survSize=Number(meth.survSize||0);
  const activeRows=rows.filter(r=>r&&r.active);
  const impossible = survSize>0 && activeRows.length>=5 && activeRows.every(r=>Number(r.flagged||0)===survSize);
  if(!impossible) return meth;
  const cleaned={...meth, removed:{...(meth.removed||{})}};
  cleaned.removed.surv=0;
  cleaned.removed.survRules={};
  cleaned.survRuleRows=rows.map(r=>({...r,flagged:0,removed:0}));
  cleaned._survSanitized=true;
  return cleaned;
}
function getHardFilterRowsFromEngine(E){
  ...
  return [
    ...
    {key:'supply_pressure',label:'Sell-Side Pressure',criteria:'Current DMI-/sell-side pressure stronger than buy-side, confirmed by weak VWAP/open/flow/retention',removed:rm.supply||0,rockets:rk.supply||[],active:true,kind:'core'},
...
function persistMethodologySnapshot(){
  ...
  try{
    const methSave={
      ...
      yesterdayRockets:ENGINE_DATA.yesterdayRockets||[],
      qualityTargetN:ENGINE_DATA.qualityTargetN||0,
...
function recordRecommendationOutcomeScan(scan){
  ...
  Object.values(store.issues||{}).forEach(issue=>{
    ...
    (issue.picks||[]).forEach(p=>{
      ...
      if(!p.rocketDate&&row.priceChange!=null&&row.priceChange>=issue.threshold){
...
function recordPostSaleRocketScan(scan){
  ...
  TRADEBOOK_STATS.tripsData.forEach((trip,idx)=>{
    ...
    if(!row||row.priceChange==null||row.priceChange<scan.threshold||store.events[key]) return;
...
function detectNSE(filename,content){
  ...
  if(fn.includes('sec_list')||fn.includes('price_band')||fn.includes('priceband')){parsePriceBand(content);return'price_band';}
  ...
  if(fn.includes('nse holidays')||fn.includes('nse_holidays')){parseNSEHolidays(content);return'holidays';}
  return null;
}
...
function runEngine(raw, sessionTag){
  ...
  // isNewDay: first upload of each trading day increments session count and EMA-blends.
  // Same-day re-uploads recompute correlations but don't increment session count.
  // No market-hours guard — lagged learning uses yesterday's snapshot regardless of upload time.
  const _today=getSessionDate();
  const _isMarket=true;
  ...
  const findKeys = pattern => Object.keys(COL_MAP).filter(k => pattern.test(k));
  const findKeyPref = (preferred, fallback) =>
    Object.keys(COL_MAP).find(k => preferred.some(p => p.test(k))) || findKey(fallback);
  const INTRADAY_TF = /(1_minute|3_minutes|5_minutes|15_minutes|30_minutes|45_minutes|1_hour|2_hours|4_hours)/;
  const K = {
    price:        findKey(/^price$/),
    ...
    change_open:  findKey(/change_from_open.*1_day|1_day.*change_from_open/),
    rvol_time:    findKey(/^relative_volume_at_time$|relative_volume.*time/),
    rvol_day:     findKey(/^relative_volume_1_day$|relative_volume.*1_day/),
    buy_qty:      findKey(/(^|_)buy(_|ing_)?(qty|quantity|volume|orders?)$|(^|_)bid(_|_qty|_quantity|_volume)/),
    sell_qty:     findKey(/(^|_)sell(_|ing_)?(qty|quantity|volume|orders?)$|(^|_)ask(_|_qty|_quantity|_volume)|offer(_|_qty|_quantity|_volume)/),
    dmi_pos:      findKeyPref([new RegExp('directional_movement_index.*'+INTRADAY_TF.source+'.*positive'),new RegExp('directional_movement_index.*positive.*'+INTRADAY_TF.source)],/directional_movement_index.*positive/),
    dmi_neg:      findKeyPref([new RegExp('directional_movement_index.*'+INTRADAY_TF.source+'.*negative'),new RegExp('directional_movement_index.*negative.*'+INTRADAY_TF.source)],/directional_movement_index.*negative/),
    adx:          findKeyPref([new RegExp('average_directional_index.*'+INTRADAY_TF.source),new RegExp(INTRADAY_TF.source+'.*average_directional_index')],/average_directional_index/),
    mfi:          findKeyPref([new RegExp('money_flow_index.*'+INTRADAY_TF.source),new RegExp(INTRADAY_TF.source+'.*money_flow_index')],/money_flow_index/),
    rsi:          findKeyPref([new RegExp('relative_strength_index.*'+INTRADAY_TF.source),new RegExp(INTRADAY_TF.source+'.*relative_strength_index')],/relative_strength_index/),
  };
  K.dmi_pos_keys = findKeys(new RegExp('directional_movement_index.*'+INTRADAY_TF.source+'.*positive')).concat(findKeys(new RegExp('directional_movement_index.*positive.*'+INTRADAY_TF.source)));
  K.dmi_neg_keys = findKeys(new RegExp('directional_movement_index.*'+INTRADAY_TF.source+'.*negative')).concat(findKeys(new RegExp('directional_movement_index.*negative.*'+INTRADAY_TF.source)));
  K.adx_keys = findKeys(new RegExp('average_directional_index.*'+INTRADAY_TF.source)).concat(findKeys(new RegExp(INTRADAY_TF.source+'.*average_directional_index')));
  K.mfi_keys = findKeys(new RegExp('money_flow_index.*'+INTRADAY_TF.source)).concat(findKeys(new RegExp(INTRADAY_TF.source+'.*money_flow_index')));
  K.rsi_keys = findKeys(new RegExp('relative_strength_index.*'+INTRADAY_TF.source)).concat(findKeys(new RegExp(INTRADAY_TF.source+'.*relative_strength_index')));

  function getSellPressureReason(d){
    const directBuy=K.buy_qty?d[K.buy_qty]:null;
    const directSell=K.sell_qty?d[K.sell_qty]:null;
    if(directBuy!=null&&directSell!=null&&directBuy>0&&directSell>directBuy*1.05){
      return `Sell side ${directSell.toFixed(0)} > buy side ${directBuy.toFixed(0)}`;
    }

    const dmiPairs=[];
    const seenPairs=new Set();
    const addPair=(posKey,negKey,weight,label)=>{
      if(!posKey||!negKey) return;
      const id=posKey+'|'+negKey;
      if(seenPairs.has(id)) return;
      seenPairs.add(id);
      dmiPairs.push({posKey,negKey,weight,label});
    };
    [
      ['5_minutes',1.35,'5m'],
      ['15_minutes',1.15,'15m'],
      ...
      ['45_minutes',0.85,'45m'],
      ['1_hour',0.75,'1h']
    ].forEach(([tf,weight,label])=>{
      const pos=(K.dmi_pos_keys||[]).find(k=>k.includes(tf));
      const neg=(K.dmi_neg_keys||[]).find(k=>k.includes(tf));
      addPair(pos,neg,weight,label);
    });
    addPair(K.dmi_pos,K.dmi_neg,0.45,'1d');

    let sellWeight=0,buyWeight=0,totalWeight=0,edgeSum=0,best=null;
    dmiPairs.forEach(p=>{
      const dp=d[p.posKey], dn=d[p.negKey];
      if(dp==null||dn==null||!isFinite(dp)||!isFinite(dn)||(dp+dn)<=0) return;
      const edge=(dn-dp)/(dn+dp);
      totalWeight+=p.weight;
      edgeSum+=edge*p.weight;
      if(edge>=SELL_PRESSURE_EDGE_MIN) sellWeight+=p.weight;
      else if(edge<=-SELL_PRESSURE_EDGE_MIN) buyWeight+=p.weight;
      if(!best||edge>best.edge) best={...p,edge,dp,dn};
    });
    const netEdge=totalWeight?edgeSum/totalWeight:0;
    const rawRvols=[K.rvol_time?d[K.rvol_time]:null,K.rvol_day?d[K.rvol_day]:null]
      .filter(v=>v!=null&&isFinite(v)&&v>0);
    const maxRvol=rawRvols.length?Math.max(...rawRvols):null;
    const buyDominant=totalWeight>0&&buyWeight>=Math.max(1.25,sellWeight+0.45)&&netEdge<=-SELL_PRESSURE_EDGE_MIN;
    if(maxRvol!=null&&maxRvol>=RVOL_PRESSURE_MIN&&totalWeight>0&&!buyDominant){
      const label=best?.label||'intraday';
      const pct=(best?.edge!=null?(best.edge*100).toFixed(1):'0.0');
      return `High RVOL ${maxRvol.toFixed(2)} without upward direction (${label} DMI edge ${pct}%)`;
    }
    if(!totalWeight||sellWeight<Math.max(1.8,buyWeight+0.45)) return '';

    const price=K.price?d[K.price]:null;
    const vwap=K.vwap?d[K.vwap]:null;
    const pc=K.price_change?d[K.price_change]:null;
    const co=K.change_open?d[K.change_open]:null;
    let confirms=0;
    if(price!=null&&vwap!=null&&vwap>0&&price<vwap*0.9985) confirms++;
    if(co!=null&&co<0) confirms++;
    if(pc!=null&&pc<0) confirms++;
    if(d.peak_retention!=null&&d.peak_retention<58) confirms++;
    const mfiKeys=(K.mfi_keys&&K.mfi_keys.length)?K.mfi_keys:[K.mfi].filter(Boolean);
    if(mfiKeys.some(k=>d[k]!=null&&d[k]<45)) confirms++;
    const rsiKeys=(K.rsi_keys&&K.rsi_keys.length)?K.rsi_keys:[K.rsi].filter(Boolean);
    if(rsiKeys.some(k=>d[k]!=null&&d[k]<45)) confirms++;

    if(confirms<1||netEdge<SELL_PRESSURE_EDGE_MIN) return '';
    const pct=(best?.edge!=null?(best.edge*100).toFixed(1):'');
    return `Sell-side pressure${best?` (${best.label} DMI- > DMI+ by ${pct}%)`:''}`;
  }
  ...
  // ── Configurable thresholds (read early so hard filters can use rocketPct) ──
  let rocketPct=10;
  const _rPctEl=document.getElementById('fRocketPct');
  const _rPctRaw=parseFloat(_rPctEl?.value);
  const ROCKET_THRESHOLD=(isFinite(_rPctRaw)&&_rPctRaw>=5&&_rPctRaw<=20)?_rPctRaw:10;
  rocketPct=ROCKET_THRESHOLD;

  // ── Hard Filters (legacy profitable engine — single pass, learning = display) ──
  // Surveillance flagging uses user's SURV_CUSTOM_RULES (via NSE_SURV[sym]) but is applied
  // as a hard exclusion from both learning and display — matching the engine that was
  // profitable for 30 days. Non-EQ series exclusion is kept on top (user choice).
  ...
  REMOVED={uc:0,surv:0,nonEq:0,liq:0,fscore:0,atr:0,deliv:0,fade:0,supply:0,survRules:{},
    rockets:{uc:[],surv:[],nonEq:[],liq:[],fscore:[],atr:[],deliv:[],fade:[],supply:[]}};
  const filtered=parsed.filter(d=>{
    ...
    const _track=(bucket)=>{
      ...
      REMOVED[bucket]++;
      if(pc!=null&&pc>=rocketPct) REMOVED.rockets[bucket].push({sym,pc});
    };
    ...
    // Sell-side pressure — avoid names where current supply is stronger than demand.
    // Uses direct buy/sell quantity columns when available, otherwise intraday DMI-/DMI+
    // with weak VWAP/open/flow/retention confirmation. Blank signals pass as unknown.
    const sellPressureReason=getSellPressureReason(d);
    if(sellPressureReason){d._sellPressureReason=sellPressureReason;_track('supply');return false;}
    ...
    // Delivery — stock-only NSE filter; blanks pass through as unknown
    const del=d.delivery_pct;
    if(del!==null&&del<30){_track('deliv');return false;}
    // Peak retention — stock has given back >50% of today's day range (fading). Only applied
    // when day range is meaningful (>1% of price) to avoid noise on tight ranges.
    const hi1d=K.high_1d?d[K.high_1d]:null, lo1d=K.low_1d?d[K.low_1d]:null;
    if(hi1d!=null&&lo1d!=null&&lo1d>0&&(hi1d-lo1d)/lo1d>0.01){
      if(d.peak_retention!=null&&d.peak_retention<50){_track('fade');return false;}
    }
  ...
  // Market hours guard: only accumulate correlations during 9:00–16:00 IST.
  ...
  // ── Yesterday's rockets — continuation-aware learning and display tracking ──
  const prevSnap=PREV_SNAP||{};
  const pcKey=K.price_change;
  const yesterdayRockets=[];
  const goodRocketsToday=new Set();
  const continuationHoldFactor=0.3;
  ...
  if(pcKey){
    Object.entries(prevSnap).forEach(([sym,feats])=>{
      ...
      const collapseThreshold=-(yesterdayChg*continuationHoldFactor);
      const held=todayChg!=null&&todayChg>collapseThreshold;
      yesterdayRockets.push({symbol:sym,yesterdayChg:+yesterdayChg.toFixed(2),todayChg:todayChg!=null?+todayChg.toFixed(2):null,held});
      if(held&&idx!=null) goodRocketsToday.add(idx);
    });
  }
  yesterdayRockets.sort((a,b)=>b.yesterdayChg-a.yesterdayChg);

  // Positive target includes fresh rockets and yesterday's rockets that held their move.
  // Display suppression remains independent, so continuation shapes rankings without forcing re-entry.
  const qualitySet=new Set([...topSetToday,...goodRocketsToday]);
  const topN=topSetToday.size;
  const continuationN=goodRocketsToday.size;
  const qualityN=qualitySet.size;

  // ── Lagged correlation: yesterday's features → quality target (binary 0/1) ──
  ...
  if(qualityN===0){
    laggedNote='No rockets or held continuations today — using historical only';
    ...
    laggedNote='Lagged ('+overlap.length+' matched · '+topN+' fresh rockets · +'+continuationN+' held continuations · '+qualityN+' quality target)';
    ...
    const priorLearnedSessions=ACC_CORR?.learnSessions??ACC_CORR?.sessions??0;
    if(priorLearnedSessions>=2){
      laggedNote=overlap.length>0
        ?'Low lagged overlap ('+overlap.length+' matched, need 100) — using historical correlations'
        :'No prior overlap — using historical correlations';
      for(const f of FEATS) targetCorrToday[f]=null;
    } else {
      // Cold-start only: same-day fallback is useful before the brain has history.
      const target=filtered.map((_,i)=>qualitySet.has(i)?1:0);
      laggedNote=overlap.length>0
        ?'Cold-start same-day fallback (only '+overlap.length+' matched, need 100+)'
  ...
  // ── Regime-agnostic simple-average (running mean) blend ──
  const hasTargetObservation=qualityN>0;
  const includeToday=isNewSession&&hasTargetObservation;
  const combinedLearnedN=ACC_CORR?(ACC_CORR.learnSessions??ACC_CORR.sessions??0):0;
  const newCombinedLearnedN=combinedLearnedN+(includeToday?1:0);
  const blend=(prior,today,n,add)=>{
    if(today===null||!add) return prior;
    if(prior===null||prior===undefined||n<=0) return today;
    return prior+(today-prior)/(n+1);
  };
  ...
  for(const f of FEATS){
    ...
    if(today!=null&&isFinite(today)&&Math.abs(today)>0.0001) freshSignalCount++;
    let combinedHist=ACC_CORR?.corr?.[f]??null;
    combinedCorr[f]=blend(combinedHist,today,combinedLearnedN,includeToday);
  }
  const useFreshCorr=freshSignalCount>0;
  const scoringSource=useFreshCorr?'fresh_lagged':'combined_history';
  for(const f of FEATS){
    const today=isNaN(targetCorrToday[f])?null:targetCorrToday[f];
  ...
  const newAcc={corr:combinedCorr,sessions:(ACC_CORR?ACC_CORR.sessions||0:0)+(isNewSession?1:0),
    learnSessions:newCombinedLearnedN,corrSchema:CORR_SCHEMA,
    lastUpdated:new Date().toISOString(),laggedNote,
    lastTag:sessionTag||null,lastDate:getSessionDate(),marketBreadth};
  ACC_CORR=newAcc;
  ...
  const results=filtered.map((d,idx)=>{
    ...
    let tier;
    if(score>=78)tier='S';else if(score>=64)tier='A';else if(score>=50)tier='B';else if(score>=38)tier='C';else tier='D';
  ...
  const activeLabModel=getRocketLabAdjustmentModel();
  if(activeLabModel){
    const km=rocketLabKeyMap(FEATS);
    const raw=results.map((r,idx)=>({r,score:rocketLabScoreRow(filtered[idx],activeLabModel,km)})).filter(x=>isFinite(x.score)&&x.score>-900);
    if(raw.length>=5){
      const vals=raw.map(x=>x.score), lo=Math.min(...vals), hi=Math.max(...vals);
      raw.forEach(x=>{
        const labScore=hi>lo?((x.score-lo)/(hi-lo))*100:50;
        x.r.baseRocketScore=x.r.rocketScore;
        x.r.rocketLabScore=+labScore.toFixed(1);
        x.r.rocketLabModel=activeLabModel;
        x.r.rocketScore=+((x.r.rocketScore*0.78)+(labScore*0.22)).toFixed(1);
      });
    }
  }
  ...
  parsed.forEach(d=>{
    ...
    for(const f of FEATS){
      ...
      const arr=_sf[f];if(!arr.length)continue;
  ...
  // Persist methodology data including interCorr matrix
  try{
    const methSave={targetCorr,targetCorrToday,interCorr,mrmr,weights,features:FEATS,labels:LABELS,top10Feats,rocketCount:ENGINE_DATA.rocketCount,rocketThreshold:ENGINE_DATA.rocketThreshold,accSessions:ENGINE_DATA.accSessions,laggedNote:ENGINE_DATA.laggedNote,
      ...
      yesterdayRockets:yesterdayRockets,continuationN,qualityTargetN:qualityN,totalParsed:totalParsed,hardFilterSchema:HARD_FILTER_SCHEMA,removed:{...REMOVED},survSize:Object.keys(NSE_SURV).length,
      avgMoveAll:todayAvgMoveAll,avgMoveUniverse:todayAvgMoveUniverse,recommendationFeedback,executedEntryFeedback,
      intradayTrajectory:ENGINE_DATA.intradayTrajectory,rocketLab:ENGINE_DATA.rocketLab,rocketLabActiveModel:ENGINE_DATA.rocketLabActiveModel};
    FS.set(modeKey(METH_STORE),methSave);
  }catch(e){console.warn('Could not save methodology data',e);}
  persistMethodologySnapshot();
  // Keep legacy call site; Rocket % controls rocket labels/counts only.
  try{onRocketPctChange(ROCKET_THRESHOLD);}catch(e){}
  return results;
}
...
function renderStats(){
  const t=ALL.length,sc=ALL.filter(s=>s.tier==='S').length,ac=ALL.filter(s=>s.tier==='A').length;
  ...
  const _rsCeiling=ENGINE_DATA?.rocketThreshold||10;
  const rocketCount=ALL.filter(s=>s.priceChange!=null&&s.priceChange>=_rsCeiling).length;
  ...
  document.getElementById('statsBar').innerHTML=`
    ...
    <div class="st"><div class="st-l">Rockets Today</div><div class="st-v" style="color:${rocketCount>0?'var(--fire)':'var(--t3)'}">${rocketCount}</div><div class="st-d">≥${_rsCeiling}%</div></div>
    <div class="st"><div class="st-l">Breadth</div><div class="st-v" style="color:var(--cyan)">${ENGINE_DATA.marketBreadth!=null?(ENGINE_DATA.marketBreadth*100).toFixed(0):(bull/t*100).toFixed(0)}%</div><div class="st-d">market context only · ${ENGINE_DATA.accSessions||0} learned sessions</div></div>${bookedCard}`;
  ...
  if(REMOVED.deliv>0){const _dR=REMOVED.rockets?.deliv||[];const _dTip=_dR.length?'Low-delivery rockets hidden: '+_dR.map(x=>x.sym+' +'+x.pc.toFixed(1)+'%').join(', '):'Delivery <30% when present — excluded from learning and display';filterPills.push(`<span class="info-pill pill-cyan" title="${escHtml(_dTip)}">🚫 ${REMOVED.deliv} low delivery removed</span>`);}
  if(REMOVED.fade>0){const _fR=REMOVED.rockets?.fade||[];const _fTip=_fR.length?'Faded rockets hidden: '+_fR.map(x=>x.sym+' +'+x.pc.toFixed(1)+'%').join(', '):'Peak retention <50% — given back half the day\'s range';filterPills.push(`<span class="info-pill pill-gray" title="${escHtml(_fTip)}">🚫 ${REMOVED.fade} fading (peak ret &lt;50%) removed</span>`);}
  if(REMOVED.supply>0){const _spR=REMOVED.rockets?.supply||[];const _spTip=_spR.length?'Sell-pressure rockets hidden: '+_spR.map(x=>x.sym+' +'+x.pc.toFixed(1)+'%').join(', '):'Current sell-side pressure stronger than buy-side; excluded from learning and display';filterPills.push(`<span class="info-pill pill-red" title="${escHtml(_spTip)}">Sell pressure ${REMOVED.supply} removed</span>`);}
  ...
  if(SUPPRESSED_YDAY_ROCKET>0){const _ydTip=(ENGINE_DATA.yesterdayRockets||[]).map(r=>`${r.symbol} +${r.yesterdayChg}% yday → ${r.todayChg!=null?r.todayChg+'% today':'not in today\'s scan'}`).join(', ');filterPills.push(`<span class="info-pill pill-orange" title="Already rocketed yesterday — move done, excluded from recommendations. ${escHtml(_ydTip)}">⏮ ${SUPPRESSED_YDAY_ROCKET} yday rockets hidden</span>`);}
...
function buildHardFilterMethodologyHTML(E){
  ...
  const totalRemoved=(REMOVED.uc||0)+(REMOVED.surv||0)+(REMOVED.nonEq||0)+(REMOVED.liq||0)+(REMOVED.fscore||0)+(REMOVED.atr||0)+(REMOVED.deliv||0)+(REMOVED.fade||0)+(REMOVED.supply||0);
  return `
...
function _renderMethodologyInner(){
  ...
  const corrSource = E.useAccCorr?'Historical correlations active':'Warming up (need ≥2 target sessions)';
  const scoreCorrSource = E.useFreshCorr
    ? `Fresh previous-day signal active (${E.freshSignalCount||0} features)`
  ...
  const lab=E.rocketLab||FS.get(modeKey(ROCKET_LAB_STORE))?.evaluations||null;
  ...
  let wtHTML=`<table class="ct"><thead><tr><th>Feature</th><th>Src</th><th>Target r (score)</th><th>Today r</th><th>Direction</th><th>Redundancy</th><th>mRMR Score</th><th class="bar-cell">Weight</th><th>Wt%</th></tr></thead><tbody>`;
  ...
  mc.innerHTML=`
    <nav style="position:sticky;top:var(--hdr-h,72px);z-index:50;background:var(--bg);padding:8px 0 10px;margin-bottom:8px;display:flex;gap:6px;flex-wrap:wrap;border-bottom:1px solid var(--border);box-shadow:0 2px 8px rgba(0,0,0,0.3);overflow-x:auto;-webkit-overflow-scrolling:touch">
      ...
      <a href="#meth-rocket-lab" onclick="event.preventDefault();scrollToSection('meth-rocket-lab')" style="padding:4px 12px;border-radius:6px;background:var(--bg-card);border:1px solid var(--border);color:var(--t2);font-size:11px;font-weight:600;text-decoration:none;cursor:pointer">Rocket Lab</a>
    ...
    <div class="m-grid">
      ...
      <div class="m-card"><h4>🚫 Hard Filters</h4><p>Removed from <strong>both learning and display</strong>: zero price, stocks at/near their NSE price band from <strong>sec_list</strong> (fallback ≥19.5% when band data is unavailable), non-EQ series (BE/BZ/SZ/SM/ST), surveillance-flagged stocks (user's rules), low liquidity when present (volume &lt;${(parseFloat(document.getElementById('fVolMult')?.value)||LIQ_MIN_VOL_DEFAULT).toLocaleString()}${parseFloat(document.getElementById('fMinTurnover')?.value)>0?' or turnover &lt;'+fmtINR(parseFloat(document.getElementById('fMinTurnover')?.value)):''}, shareholders &lt;500), zero Piotroski F-Score, ATR &lt;=0 when present, <strong>delivery &lt;30%</strong> when present, and <strong>peak retention &lt;50%</strong> (fading; only when day range &gt;1%). Blank/null values pass through as unknown. Currently filtered: ${(REMOVED.uc||0)+(REMOVED.surv||0)+(REMOVED.nonEq||0)+(REMOVED.liq||0)+(REMOVED.fscore||0)+(REMOVED.atr||0)+(REMOVED.deliv||0)+(REMOVED.fade||0)} stocks.</p></div>
...
function isBuyZone(s){
  if(s.price===0) return false;
  if(s.price==null||s.priceChange==null) return true;
  const maxEntry=getMaxEntry(s);
  if(maxEntry===null) return false;
  const refPrice=Math.max(s.price||0,getBuyPrice(s)||0);
  return refPrice<=maxEntry+PRICE_COMPARE_EPS;
}
...
function computeAlloc(capital, selList){
  ...
  const _topupPctEl=document.getElementById('fTopupAlloc');
  const _topupMult=Math.min(1,Math.max(0.1,((_topupPctEl?parseFloat(_topupPctEl.value)||50:50)/100)));
  const _effScore=s=>s._isTopUp?(s.rocketScore||0)*_topupMult:(s.rocketScore||0);
  const totalScore = selList.reduce((s,x)=>s+_effScore(x),0)||1;
  const allocMap = {};
  // Sort a working copy by effective score descending
  const sortedSel=[...selList].sort((a,b)=>_effScore(b)-_effScore(a));
...
function getBasketPositionSize(){
  ...
  if(capital>0&&selList.length){
    const allocMap=computeAlloc(capital,selList);
    const vals=Object.values(allocMap).map(a=>a.alloc||0).filter(v=>v>0);
    if(vals.length){
      const avg=Math.round(meanArr(vals));
      return {value:avg,source:`Current basket avg · ${vals.length} allocated`};
    }
    const fallback=Math.round(maxAlloc>0?Math.min(maxAlloc,capital):capital/selList.length);
    if(fallback>0) return {value:fallback,source:maxAlloc>0?'Max Alloc setting':'Capital ÷ selected stocks'};
  }
  if(maxAlloc>0) return {value:Math.round(maxAlloc),source:'Max Alloc setting'};
  return {value:null,source:'Set Capital/Max Alloc to recommend'};
}
...
function scC(s){return s>=78?'#fb923c':s>=64?'var(--green)':s>=50?'var(--blue)':s>=38?'var(--orange)':'var(--t3)';}
function fDel(v){
...
function applyFilters(){
  ...
  // Suppress yesterday's rockets — move already happened, system purpose is prediction not confirmation
  SUPPRESSED_YDAY_ROCKET=0;
  {
    const ydaySyms=new Set((ENGINE_DATA.yesterdayRockets||[]).map(r=>r.symbol));
    if(ydaySyms.size>0){
      FILT=FILT.filter(x=>{
        if(ydaySyms.has(x.symbol)){hiddenReasons[x.symbol]='Yesterday rocket hidden';SUPPRESSED_YDAY_ROCKET++;return false;}
        return true;
      });
    }
  }
...
function clearFilters(){
  ...
  const rocketEl=document.getElementById('fRocketPct');if(rocketEl)rocketEl.value='10';
...
function importBrain(event){
  ...
  reader.onload=async function(e){
    try{
      ...
      const isRawBrain=!!(brain&&typeof brain==='object'&&(brain[CORR_STORE]||brain[ALL_STORE]||brain[TRADEBOOK_STORE]||brain[SNAP_KEY]));
...
function resetBrain(btn){
  ...
  if(btn._stage===1){
    ...
    btn._stage=2;
    btn.innerHTML=`🗑 CONFIRM: clear ${s} learned sessions?`;
  ...
  // Stage 3: keep only a valid prior-session comparison baseline; wipe learned/runtime state.
  const preserved={};
  const stockBaseline=getPriorSessionSnapshotRecordForMode('stock');
  if(stockBaseline) preserved[SNAP_KEY]=stockBaseline;
  FS.reset(preserved);
  localStorage.removeItem(SCANNER_STORE);
  ...
  const kept=[stockBaseline&&('Stocks '+stockBaseline.savedDate)].filter(Boolean);
  const baselineMsg=kept.length?' Kept prior-session baseline: '+kept.join(', ')+'.':' No valid prior-session baseline was available.';
  showToast('<strong>Brain reset.</strong> Cleared accumulated learning and saved filters.'+baselineMsg+' Uploaded input files remain in Google Drive.',7000);
...
function captureScannerRuntime(){
  return {
    mode:MARKET_MODE,ALL,FILT,ENGINE_DATA,PREV_SNAP,PREV_SNAP_META,
    ACC_CORR,
    ...
    lastEngineFeats:window._lastEngineFeats,lastParsedFiltered:window._lastParsedFiltered
  };
}
function restoreScannerRuntime(s){
  MARKET_MODE=s.mode;ALL=s.ALL;FILT=s.FILT;ENGINE_DATA=s.ENGINE_DATA;PREV_SNAP=s.PREV_SNAP;PREV_SNAP_META=s.PREV_SNAP_META;
  ACC_CORR=s.ACC_CORR;
  ...
  window._lastEngineFeats=s.lastEngineFeats;window._lastParsedFiltered=s.lastParsedFiltered;
}
function normSnapshotDate(s){
  if(!s) return null;
...
function getPriorSessionSnapshotRecordForMode(mode){
  const todayISO=getSessionDate();
  const snap=FS.get(modeKey(SNAP_KEY,mode)), prev=FS.get(modeKey(SNAP_PREV_KEY,mode));
  const candidates=[snap,prev]
    .filter(s=>s&&s.stocks&&normSnapshotDate(s.savedDate)&&normSnapshotDate(s.savedDate)<todayISO)
    .sort((a,b)=>normSnapshotDate(b.savedDate).localeCompare(normSnapshotDate(a.savedDate)));
  return candidates.find(s=>tradingDaysBetween(normSnapshotDate(s.savedDate),todayISO)===1)||null;
}
function loadPrevSnapForMode(mode){
  const source=getPriorSessionSnapshotRecordForMode(mode);
  const out={snap:{},meta:{}};
  ...
  } else out.snap=source.stocks||source;
  out.meta={savedDate:source.savedDate||null,marketBreadth:source.marketBreadth??null};
  return out;
}
function compactFeatureValue(v){
  if(v===null||v===undefined||isNaN(v)) return null;
  return +parseFloat(v).toFixed(3);
}
function buildScannerStockMatrix(rows, cols){
  const stocks={};
  rows.forEach(d=>{
    const sym=normSym(d.symbol);
    if(!sym) return;
    stocks[sym]=cols.map(f=>compactFeatureValue(d[f]));
  });
  return stocks;
}
function getRocketLabStore(){
  const store=FS.get(modeKey(ROCKET_LAB_STORE))||{};
  const sessions=Array.isArray(store.sessions)?store.sessions:[];
  const evaluations=store.evaluations&&typeof store.evaluations==='object'?store.evaluations:null;
  return {version:1,sessions,evaluations,activeModel:store.activeModel||null,lastUpdated:store.lastUpdated||null};
}
function featureKeyByPatterns(cols,patterns){
...
function buildRocketLabSession({date,threshold,cols,rows,labels,marketBreadth,totalParsed}){
  ...
  rows.forEach(r=>{
    const sym=normSym(r.symbol);
    if(!sym) return;
    data.push([sym,compactFeatureValue(r.rocketScore),r._hardFiltered?0:1,...cols.map(c=>compactFeatureValue(r[c]))]);
  });
  return {date,threshold,cols,labels:cols.reduce((o,c)=>{o[c]=labels?.[c]||c;return o;},{}),marketBreadth,totalParsed,rows:data};
}
function rocketLabRows(session){
  if(!session||!Array.isArray(session.rows)) return [];
  const cols=session.cols||[];
  return session.rows.map(arr=>{
    const r={symbol:arr[0],baseScore:arr[1],eligible:arr[2]===1};
    cols.forEach((c,i)=>{r[c]=arr[i+3];});
    return r;
  });
}
function rocketLabKeyMap(cols){
  return {
    price:featureKeyByPatterns(cols,[/^price$/]),
    pc:featureKeyByPatterns(cols,[/price_change.*1_day|1_day.*price_change|^change.*1_day$/]),
    open:featureKeyByPatterns(cols,[/change_from_open.*1_day|1_day.*change_from_open/]),
    rvolTime:featureKeyByPatterns(cols,[/relative_volume.*time/]),
    rvolDay:featureKeyByPatterns(cols,[/relative_volume.*1_day/]),
    turnover:featureKeyByPatterns(cols,[/price_volume_turnover_1_day/]),
    atr:featureKeyByPatterns(cols,[/average_true_range.*1_day|atr.*1_day/]),
    dmi5p:featureKeyByPatterns(cols,[/directional_movement_index.*5_minutes.*positive/]),
    dmi5n:featureKeyByPatterns(cols,[/directional_movement_index.*5_minutes.*negative/]),
    dmi15p:featureKeyByPatterns(cols,[/directional_movement_index.*15_minutes.*positive/]),
    dmi15n:featureKeyByPatterns(cols,[/directional_movement_index.*15_minutes.*negative/]),
    mfi5:featureKeyByPatterns(cols,[/money_flow_index.*5_minutes/]),
    mfi15:featureKeyByPatterns(cols,[/money_flow_index.*15_minutes/]),
    rsi5:featureKeyByPatterns(cols,[/relative_strength_index.*5_minutes/]),
    rsi15:featureKeyByPatterns(cols,[/relative_strength_index.*15_minutes/]),
    roc5:featureKeyByPatterns(cols,[/rate_of_change.*5_minutes/]),
    delivery:cols.includes('delivery_pct')?'delivery_pct':null,
    peak:cols.includes('peak_retention')?'peak_retention':null,
    range:cols.includes('range_pos')?'range_pos':null,
    from52:cols.includes('pct_from_52w_high')?'pct_from_52w_high':null,
    sectorRel:cols.includes('sector_rel_strength')?'sector_rel_strength':null,
    industryBreadth:cols.includes('industry_breadth')?'industry_breadth':null,
  };
}
function rocketLabVal(r,k){return k?r[k]:null;}
function rocketLabScoreRow(r,model,km){
  const eligible=r.eligible!==false&&!r._hardFiltered;
  const pc=rocketLabVal(r,km.pc), open=rocketLabVal(r,km.open);
  const rvol=Math.max(rocketLabVal(r,km.rvolTime)||0,rocketLabVal(r,km.rvolDay)||0);
  const peak=rocketLabVal(r,km.peak), del=rocketLabVal(r,km.delivery), sec=rocketLabVal(r,km.sectorRel);
  const d5p=rocketLabVal(r,km.dmi5p), d5n=rocketLabVal(r,km.dmi5n), d15p=rocketLabVal(r,km.dmi15p), d15n=rocketLabVal(r,km.dmi15n);
  const mfi=Math.max(rocketLabVal(r,km.mfi5)||0,rocketLabVal(r,km.mfi15)||0), rsi=Math.max(rocketLabVal(r,km.rsi5)||0,rocketLabVal(r,km.rsi15)||0);
  const from52=rocketLabVal(r,km.from52), range=rocketLabVal(r,km.range);
  const dmiEdge=((d5p||0)-(d5n||0))*0.6+((d15p||0)-(d15n||0))*0.4;
  const cap=(v,lo,hi)=>Math.max(lo,Math.min(hi,v));
  if(model==='current') return r.baseScore??0;
  if(!eligible) return -999;
  if(pc==null) return -999;
  if(model==='rvol_early'){
    const chasePenalty=pc>5?(pc-5)*18:pc< -2?Math.abs(pc+2)*8:0;
    return cap(rvol,0,20)*8 + dmiEdge*0.7 + (peak??50)*0.35 + (del??35)*0.2 + (sec??0)*2 - chasePenalty;
  }
  if(model==='quiet_accum'){
    const quiet=pc<=3?25:pc<=5?8:-(pc-5)*16;
    const coil=from52!=null?cap(Math.abs(from52),0,40)*0.4:0;
    return quiet + cap(rvol,0,12)*7 + dmiEdge*0.6 + (del??35)*0.35 + (peak??50)*0.25 + coil + (sec??0)*1.5;
  }
  if(model==='sector_breakout'){
    const entry=pc<=6?10:-(pc-6)*14;
    return entry + (sec??0)*5 + (rocketLabVal(r,km.industryBreadth)??50)*0.25 + dmiEdge*0.8 + cap(rvol,0,15)*5 + (peak??50)*0.2;
  }
  if(model==='durable_momo'){
    const notTooHot=pc<=8?12:-(pc-8)*15;
    return notTooHot + (peak??50)*0.55 + dmiEdge*0.9 + cap(rvol,0,20)*5 + Math.max(0,(mfi||50)-45)*0.6 + Math.max(0,(rsi||50)-50)*0.4 + (del??35)*0.25;
  }
  return 0;
}
function evaluateRocketLabSessions(store){
  const sessions=(store.sessions||[]).slice().sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  const models=['current','rvol_early','quiet_accum','sector_breakout','durable_momo'];
  const metrics={};
  models.forEach(m=>metrics[m]={model:m,days:0,rockets:0,hit20:0,hit50:0,p20:0,p50:0,avgNextHigh:null});
  for(let i=0;i<sessions.length-1;i++){
    const s=sessions[i], n=sessions[i+1];
    const today=rocketLabRows(s), next=rocketLabRows(n), km=rocketLabKeyMap(s.cols||[]), nkm=rocketLabKeyMap(n.cols||[]);
    const nextPcKey=nkm.pc;
    if(!today.length||!next.length||!nextPcKey) continue;
    const nextMap={};next.forEach(r=>{nextMap[r.symbol]=r;});
    const rockets=new Set(next.filter(r=>(rocketLabVal(r,nextPcKey)??-999)>=(n.threshold||10)).map(r=>r.symbol));
    if(!rockets.size) continue;
    models.forEach(model=>{
      const ranked=today
        .map(r=>({sym:r.symbol,score:rocketLabScoreRow(r,model,km)}))
        .filter(x=>isFinite(x.score))
        .sort((a,b)=>b.score-a.score);
      const top20=ranked.slice(0,20).map(x=>x.sym), top50=ranked.slice(0,50).map(x=>x.sym);
      const h20=top20.filter(sym=>rockets.has(sym)).length, h50=top50.filter(sym=>rockets.has(sym)).length;
      const m=metrics[model];m.days++;m.rockets+=rockets.size;m.hit20+=h20;m.hit50+=h50;m.p20+=top20.length?h20/top20.length:0;m.p50+=top50.length?h50/top50.length:0;
    });
  }
  Object.values(metrics).forEach(m=>{
    m.recall20=m.rockets?+(m.hit20/m.rockets*100).toFixed(1):0;
    m.recall50=m.rockets?+(m.hit50/m.rockets*100).toFixed(1):0;
    m.precision20=m.days?+(m.p20/m.days*100).toFixed(1):0;
    m.precision50=m.days?+(m.p50/m.days*100).toFixed(1):0;
  });
  const ranked=Object.values(metrics).sort((a,b)=>(b.recall20-a.recall20)||(b.recall50-a.recall50)||(b.precision20-a.precision20));
  const current=metrics.current;
  const best=ranked.find(m=>m.model!=='current'&&m.days>=1&&(m.recall20>current.recall20||m.recall50>current.recall50))||null;
  return {models:metrics,ranked,bestModel:best?.model||null,days:Math.max(...Object.values(metrics).map(m=>m.days),0),updated:getSessionDate()};
}
function buildRocketLabSeedSessionFromPrevSnap({date,threshold,cols,labels,prevSnap}){
...
function updateRocketLab(session,seedSession){
  ...
  const store=getRocketLabStore();
  const sessions=store.sessions.filter(s=>s.date!==session.date);
  if(seedSession&&!sessions.some(s=>s.date===seedSession.date)&&seedSession.date!==session.date){
    sessions.push(seedSession);
  }
  sessions.push(session);
  sessions.sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  const trimmed=sessions.slice(-15);
  const evaluations=evaluateRocketLabSessions({...store,sessions:trimmed});
  const next={version:1,sessions:trimmed,evaluations,activeModel:evaluations.bestModel||null,lastUpdated:new Date().toISOString()};
  FS.set(modeKey(ROCKET_LAB_STORE),next);
  return next;
}
function getRocketLabAdjustmentModel(){
  const lab=FS.get(modeKey(ROCKET_LAB_STORE));
  const ev=lab?.evaluations, model=lab?.activeModel;
  if(!ev||!model||!ev.models?.[model]) return null;
  const m=ev.models[model], cur=ev.models.current||{};
  if((m.days||0)<1) return null;
  if((m.recall20||0)<=(cur.recall20||0)&&(m.recall50||0)<=(cur.recall50||0)) return null;
  return model;
}
...
function applySavedFiltersForMode(mode){
  const ids=['fMinScore','fPriceMin','fPriceMax','fMin1D','fMax1D','fVol','fVolMult','fMinTurnover','fMinMarketCap','fCapital','fMaxAlloc','fReDrop','fTopupAlloc','fRocketPct'];
  const prev={};
  ...
  try{
    ...
    const map={minScore:'fMinScore',priceMin:'fPriceMin',priceMax:'fPriceMax',fMin1D:'fMin1D',fMax1D:'fMax1D',fvol:'fVol',volMult:'fVolMult',minTurnover:'fMinTurnover',minMarketCap:'fMinMarketCap',reDrop:'fReDrop',topupAlloc:'fTopupAlloc',rocketPct:'fRocketPct'};
...
async function processScannerUpload(scannerFile, mode){
  ...
  let completed=false;
  MARKET_MODE=mode;
  const prev=loadPrevSnapForMode(mode);
  PREV_SNAP=prev.snap;PREV_SNAP_META=prev.meta;
  ...
  try{
    ...
    const ok=fname.includes('all nse');
    ...
    const priorCorr=FS.get(modeKey(CORR_STORE,mode));
    const isDuplicateSession=!!(priorCorr&&priorCorr.lastTag===sessionTag);
    ...
    if(window._lastEngineFeats&&window._lastParsedFiltered){
      const cols=window._lastEngineFeats, stocks={};
      Object.assign(stocks,buildScannerStockMatrix(window._lastParsedFiltered,cols));
      const today=getSessionDate(), existingSnap=FS.get(modeKey(SNAP_KEY,mode)), updates={};
      const existingDate=existingSnap?.savedDate, isLegacyFormat=existingDate&&/^[A-Z][a-z]{2} /.test(existingDate);
      ...
      saveIntradayLedgerForMode(mode,cols,stocks,sessionTag);
    }
    if(mode==='stock'){
      const threshold=ENGINE_DATA?.rocketThreshold||10;
      const eligibleCandidates=getDisplayedEntryCandidates(ALL)
        .filter(s=>s.price>0&&(s.priceChange==null||s.priceChange<threshold));
...
async function processFiles(files){
  ...
  for(const f of files){
    ...
    if(fname_l.includes('nse holidays')||fname_l.includes('nse_holidays')){
      const ht=await f.text();parseNSEHolidays(ht);holidayFile=true;
      continue;
    }
  }
...
async function initApp(){
  ...
  if(brain){
    ...
    // Load PREV_SNAP from brain — discard if more than 1 trading day old (stale lagged data)
    try{
      const source=getPriorSessionSnapshotRecordForMode(MARKET_MODE);
      if(source){
        if(source.cols){
          Object.entries(source.stocks).forEach(([sym,vals])=>{
            const obj={};source.cols.forEach((f,i)=>obj[f]=vals[i]);
            const ns=normSym(sym);
            if(ns) PREV_SNAP[ns]=obj;
          });
        } else {
          PREV_SNAP={};
          Object.entries(source.stocks||source).forEach(([sym,feats])=>{
            const ns=normSym(sym);
            if(ns&&feats&&typeof feats==='object') PREV_SNAP[ns]=feats;
          });
        }
        PREV_SNAP_META={savedDate:source.savedDate||null,marketBreadth:source.marketBreadth??null};
        console.log('INIT: loaded prev-day snapshot from',source.savedDate,',',Object.keys(PREV_SNAP).length,'stocks');
      }
    }catch(e){PREV_SNAP={};PREV_SNAP_META={};}
    ...
    try{
      ...
      if(meth&&meth.features&&meth.features.length){
        ENGINE_DATA={...meth,interCorr:meth.interCorr||{}};
        COLS=getCols();
        ...
        if(meth.removed&&typeof meth.removed==='object'){
          ...
          REMOVED={uc:0,surv:0,nonEq:0,survRules:{},liq:0,fscore:0,atr:0,deliv:0,fade:0,supply:0,rockets:{uc:[],surv:[],nonEq:[],liq:[],fscore:[],atr:[],deliv:[],fade:[],supply:[]},...restored};
...
function loadFilterState(){
  try{
    ...
    if(state.rocketPct!=null){const el=document.getElementById('fRocketPct');if(el)el.value=state.rocketPct;}
